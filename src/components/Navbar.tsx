...
const navItems = [
  {
    label: "Home",
    items: [
      { title: "Introduction", href: "/docs", description: "Reusable components built with Radix UI + Tailwind." },
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Progress", href: "/docs/primitives/progress", description: "Displays progress of a task as a bar." },
    ],
  },
  {
    label: "Docs",
...
export function Navbar() {
  return (
    <div className="sticky top-0 z-50 w-full border-b border-t bg-background/70 backdrop-blur-md">
      <div className="container flex h-16 items-center justify-between">
        {/* Logo */}
        <Link to="/" className="hidden font-bold text-lg md:ml-auto md:block md:mr-0  ">
          MyApp
        </Link>

        {/* Desktop Navigation */}
        <div className="hidden w-full md:flex items-center gap-6">
          <NavigationMenu>
            <NavigationMenuList>
              {navItems.map((item) =>
                item.items ? (
                ...
                {navItems.map((item) =>
                  item.items ? (
                    <div key={item.label} className="flex flex-col gap-2">
                      ...
                      {item.items.map((sub) => (
                        <Link key={sub.title} to={sub.href} className="pl-4 text-sm text-muted-foreground hover:text-foreground">
                          {sub.title}
                        </Link>
                      ))}
                    </div>
